...
function loadAndInitNotifications() {
    notificationModel.getNotifications((result) => {    
        ...
        if (result.notifications && Array.isArray(result.notifications.notifications)) {
            activeNotifications = initNotifications(result.notifications.notifications);
        } else {
            // Handle the case when notifications property is missing or not an array
            activeNotifications = []; // or any other appropriate action
        }
